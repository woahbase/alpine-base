...
OSVERSION ?= $(call get_os_version)
# QEMUVERS  ?= 9.2.2#$(call get_gh_version,tonistiigi/binfmt,QEMUVERSION)#$(call get_gh_version,multiarch/qemu-user-static,QEMUVERSION)
...
LABELFLAGS ?= \
	...
	--label online.woahbase.os-version=$(OSVERSION) \
	--label online.woahbase.os=$(OPSYS) \
...
# {{{ -- image targets
build : inbinfmt
build : fetch # fetchqemu # additional build requirements
...
push : SKIP_VERSIONTAG ?= $(if $(OSVERSION),,1)
push : VERSIONTAG ?= $(subst $(ARCH),$(ARCH)$(if $(OSVERSION),_$(OSVERSION),),$(IMAGETAG))
push : BUILDDATETAG ?= $(subst $(ARCH),$(ARCH)$(if $(OSVERSION),_$(OSVERSION),)_$(BUILDDATE),$(IMAGETAG))
push : ## push image
	if [ -z "$(SKIP_$(ARCH))" ]; \
	then \
		if [ -z "$(SKIP_LATESTTAG)" ]; \
		...
		if [ -z "$(SKIP_VERSIONTAG)" ] && [ -n "$(OSVERSION)" ];\
		then \
...
# push image with version tag (if available)
# to skip pushing version set SKIP_VERSIONTAG to non empty string e.g. 1, default is unset if version given
push_registry_% : SKIP_VERSIONTAG ?= $(if $(OSVERSION),,1)
push_registry_% : REGDSTTAG ?= $(subst $(REGISTRY),$(subst push_registry_,,$@),$(IMAGETAG))
push_registry_% : REGDSTVERSIONTAG ?= $(subst $(ARCH),$(ARCH)_$(OSVERSION),$(REGDSTTAG))
push_registry_% : REGDSTBUILDDATETAG ?= $(subst $(ARCH),$(ARCH)$(if $(OSVERSION),_$(OSVERSION),)_$(BUILDDATE),$(REGDSTTAG))
...
annotate_version : TAGNAME ?= $(if $(OSVERSION),$(OSVERSION),$(error OSVERSION is not defined))
annotate_version : manifest annotate ## annotate and push `VERSION` tag

annotate_date : SKIP_ANNOTATE ?= $(if $(SKIP_BUILDDATETAG),1,)
annotate_date : TAGNAME ?= $(if $(OSVERSION),$(OSVERSION)_,$(error OSVERSION is not defined))$(BUILDDATE)
...
fetch : ## fetch alpine rootfs
	mkdir -p data && cd data \
	&& OSARCH="$(call get_alpine_rootfs_type)" \
	&& OSVERSION="$(OSVERSION)" \
	&& OSVESHORT="$$(v=$(OSVERSION); echo $${v%.*}; unset v;)" \
	&& echo "Using os arch   : "$${OSARCH} \
	&& echo "Using os version: "$${OSVERSION} \
	&& curl \
		-o ./rootfs-$${OSARCH}.tar.gz \
		-jkSL https://dl-cdn.alpinelinux.org/alpine/v$${OSVESHORT}/releases/$${OSARCH}/alpine-minirootfs-$(OSVERSION)-$${OSARCH}.tar.gz \
		&& gunzip -f ./rootfs-$${OSARCH}.tar.gz; \
		# -jkSL https://nl.alpinelinux.org/alpine/latest-stable/releases/$${OSARCH}/alpine-minirootfs-$(OSVERSION)-$${OSARCH}.tar.gz \
		#

# fetchqemu : ## fetch qemu binaries
# 	if [ "$(ARCH)" != "$(HOSTARCH)" ]; then \
# 		mkdir -p data \
# 		&& QEMUARCH="$(call get_qemu_platform)" \
...
# 			-jkSL https://github.com/multiarch/qemu-user-static/releases/download/v$(QEMUVERS)/$(HOSTARCH)_qemu-$${QEMUARCH}-static.tar.gz \
...
# which alpine rootfs to download when building for $(ARCH)
ALPINE_ROOTFS_MAP := \
	'aarch64' ) echo -n 'aarch64'    ;; \
	'armhf'   ) echo -n 'armhf'      ;; \
	'armv7l'  ) echo -n 'armv7'      ;; \
	...
	'x86_64'  ) echo -n 'x86_64'     ;; \
	#
define get_alpine_rootfs_type
$(shell case "$(ARCH)" in $(ALPINE_ROOTFS_MAP) esac)
endef

# sets qemu platform when building from $(ARCH)
QEMU_PLATFORM_MAP := \
	'aarch64' ) echo -n 'aarch64'    ;; \
	'armhf'   ) echo -n 'arm'        ;; \
	'armv7l'  ) echo -n 'armeb'      ;; \
	...
	'x86_64'  ) echo -n 'x86_64'     ;; \
	#
define get_qemu_platform
$(shell case "$(ARCH)" in $(QEMU_PLATFORM_MAP) esac)
endef
...
# gets latest release version from https://nl.alpinelinux.org
...
define get_os_version
$(shell \
	if [ ! -f $(CURDIR)/OSVERSION ] \
	|| [ "$$(( ($$(date +%s) - $$(stat $(CURDIR)/OSVERSION  -c %Y)) / 3600 ))" -gt 8 ]; \
	then \
		curl -jsSL \
		https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/$(call get_alpine_rootfs_type)/ \
		2>/dev/null \
		| grep -Eo   '>alpine-minirootfs-([0-9\.]+)-$(call get_alpine_rootfs_type).tar.gz' \
		| sed  -Ee 's!>alpine-minirootfs-([0-9\.]+)-$(call get_alpine_rootfs_type).tar.gz!\1!' \
		| sort -u \
		| tail -n 1 > $(CURDIR)/OSVERSION;
	fi;
	cat $(CURDIR)/OSVERSION;)
endef
# grep -oP "(?<=alpine-minirootfs-)[0-9\.]+(?=-$(call get_alpine_rootfs_type).tar.gz)" does not work in alpine (no -P)
# replaced by grep + sed

# gets latest release version from github (for qemu)
